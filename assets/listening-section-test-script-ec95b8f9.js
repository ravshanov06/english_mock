    ...
    // Correct Answers (provided section content)
    const correctAnswers = {
        1: "0795847222",
        2: "thursday",
        3: "7",
        4: ["22 november", "november 22", "22nd november", "november 22nd"],
        5: "wedding",
        6: "cheese",
        7: "india",
        8: "business",
    ...
    const testData = [
        {
            title: "SECTION 1",
            instructions: "Complete the customer booking request form. Write NO MORE THAN TWO WORDS AND/OR A NUMBER for each answer.",
            description: "Bakewell Travel - Customer Booking Request Form",
            questionRange: "1-10"
        }
    ];
    ...
    function syncTextInputsToAnswers() {
        document.querySelectorAll('input[type="text"][id^="q"]').forEach(input => {
            const qId = Number(String(input.id || '').replace('q', ''));
            if (!qId) return;
    ...
    function showCorrectAnswers() {
        ...
        results.forEach(({ questionId, userAnswer, correctAnswer, isCorrect }) => {
            ...
            if (input) {
                ...
                container = zone ? zone.parentNode : null;
            } else if ((questionId >= 11 && questionId <= 14) || (questionId >= 17 && questionId <= 26)) {
                const selected = document.querySelector(`input[name="q${questionId}"]:checked`);
                container = selected
                    ? selected.closest('.question-item')
                    : (function (node) { return node && node.closest ? node.closest('.question-item') : null; })(document.querySelector(`input[name="q${questionId}"]`));
            }
            if (!container) return;
    ...
    function highlightAnswers() {
        ...
        for (let i = 1; i <= 40; i++) {
            ...
            if (input) {
                ...
                const user = normalizeAnswerValue((liveInputValue || answers[i]) || '');
    ...
    function calculateResults() {
        ...
        for (let i = 1; i <= totalQuestions; i++) {
            const liveInputValue = (function (node) { return node ? node.value : ''; })(document.getElementById(`q${i}`));
            const userAnswer = ((liveInputValue || answers[i]) || '').trim();
    ...
    function showResults(results) {
        ...
        document.getElementById('part1Score').textContent = `${partScore}/${partTotal}`;
