        ...
        const TEST_STORAGE_ID = 'reading_reading_passage_2s_passage_2_5';
        const TEST_LABEL = 'Passage 2.5';
...
function normalizeSummaryGroups() {
    document.querySelectorAll('.question-group').forEach(group => {
        ...
        el('submit-all-btn').addEventListener('click', () => {
            ...
            const results = {};
            const groupedSherlockChoices = Array.from(document.querySelectorAll('input[name="q20_21_choice"]'));
            const groupedSherlockSelected = groupedSherlockChoices.filter(cb => cb.checked).map(cb => cb.value);
            ...
            for (let q = firstQuestion; q <= lastQuestion; q++) {
                ...
                if (q >= 20 && q <= 21 && groupedSherlockChoices.length) {
                    user = groupedSherlockSelected.join(',') || null;
                    isCorrect = groupedSherlockSelected.includes(String(corr).toUpperCase());
                    results[key] = { userAnswer: user || 'No answer', correct: corr, isCorrect };
                    if (q === 21) groupedSherlockChoices.forEach(cb => cb.disabled = true);
                    if (isCorrect) score++;
                    continue;
                }
        ...
        function showCorrectIndicators(results) {
            ...
            const groupedSherlockReview = el('q20-21-review');
            if (groupedSherlockReview) groupedSherlockReview.innerHTML = '';
            ...
            for (let q = firstQuestion; q <= total; q++) {
                ...
                if ((q === 20 || q === 21) && groupedSherlockReview) {
                    ...
                    groupedSherlockReview.appendChild(box);
                    continue;
                }
