...
const TEST_STORAGE_ID = 'listening_test_38';
...
function restoreDropZoneAnswer(questionId, answerValue) {
    ...
    const scopedOptionPrefix = questionId >= 21 ? 'opt21' : (questionId >= 17 ? 'opt17' : '');
    const normalizedAnswer = String(answerValue).toUpperCase();
    const items = Array.from(document.querySelectorAll('.drag-item'));
    const item = items.find((node) =>
        scopedOptionPrefix && (node.dataset.option || '').toUpperCase() === `${scopedOptionPrefix}${normalizedAnswer}`.toUpperCase()
    ) || items.find((node) =>
...
function restoreSavedAnswers() {
    ...
    updateProgress();
            ...
            const audioSequence = [
                "TEST 38 section 1.mp3",
                "TEST 38 section 2.mp3",
                "TEST 38 section 3.mp3",
                "TEST 38 section 4.mp3"
            ];
            ...
            function triggerAudioPlayback() {
                ...
                if (playPromise && typeof playPromise.catch === 'function') {
                    playPromise.catch(() => {
                        ...
                        setTimeout(() => {
                            const retryPromise = testAudioElement.play();
            ...
            correctAnswers[1] = 'south-west|southwest|south west';
            correctAnswers[2] = 'double';
            correctAnswers[3] = 'an office|office';
            correctAnswers[4] = 'lounge';
            correctAnswers[5] = 'A';
            correctAnswers[6] = 'B';
            correctAnswers[7] = 'B';
            correctAnswers[8] = 'amyes|amies';
            correctAnswers[9] = 'thursday';
            correctAnswers[10] = '3 hours|three hours';
            correctAnswers[11] = 'A';
            correctAnswers[12] = 'B';
            correctAnswers[13] = 'A';
            correctAnswers[14] = 'C';
            ...
            correctAnswers[29] = 'C';
            correctAnswers[30] = 'A';
            correctAnswers[31] = 'business management';
            correctAnswers[32] = 'phone interviews';
            correctAnswers[33] = 'qualification';
            correctAnswers[34] = 'public';
            correctAnswers[35] = 'salary';
            correctAnswers[36] = 'team';
            correctAnswers[37] = 'problem solving|problem-solving';
            correctAnswers[38] = 'presentations';
            correctAnswers[39] = 'essay writing|essay-writing';
            correctAnswers[40] = 'job';
            ...
            const testData = [
                {
                    title: "SECTION 1",
                    instructions: "Questions 1-10: Homecare Cleaning Agency",
                    description: "Q1-4 and Q8-10 form completion; Q5-7 multiple choice.",
                    questionRange: "1-10"
                },
                {
                    title: "SECTION 2",
                    instructions: "Questions 11-20: Museum tour",
                    description: "Q11-16 multiple choice and Q17-20 matching.",
                    questionRange: "11-20"
                },
                {
                    title: "SECTION 3",
                    instructions: "Questions 21-30: Analysis methods",
                    description: "Q21-25 matching and Q26-30 multiple choice.",
                    questionRange: "21-30"
                },
                {
                    title: "SECTION 4",
                    instructions: "Questions 31-40: Employment Survey on Graduates",
                    description: "Complete the notes below. NO MORE THAN TWO WORDS.",
                    questionRange: "31-40"
                }
            ];
            const partRanges = [
            ...
            const radioQuestionIds = [5, 6, 7, 11, 12, 13, 14, 15, 16, 26, 27, 28, 29, 30];
            const multiSelectQuestionIds = [];
            const dropZoneQuestionIds = [17, 18, 19, 20, 21, 22, 23, 24, 25];
            ...
            function checkAnswers() {
                ...
                for (let i = 1; i <= 40; i++) {
                    ...
                    if (!rawUserAnswer && dropZoneQuestionIds.includes(i)) {
                        const dropZone = document.querySelector(`.drop-zone[data-target="q${i}"]`);
                        const zoneValue = dropZone && dropZone.dataset ? (dropZone.dataset.value || '') : '';
            ...
            function findFeedbackContainer(questionId) {
                ...
                if (dropZoneQuestionIds.includes(questionId)) {
                    const dropZone = document.querySelector(`.drop-zone[data-target="q${questionId}"]`);
                    return dropZone && dropZone.closest ? dropZone.closest('.drop-item') : null;
                }
                const field = document.getElementById(`q${questionId}`);
                return field ? field.parentNode : null;
            }
            ...
            function setupDragDrop() {
                ...
                zones.forEach(zone => {
                    ...
                    zone.addEventListener('dragstart', (e) => {
                        ...
                        const valueNode = zone.querySelector('.drop-value');
                        const label = (valueNode ? valueNode.textContent.trim() : '') || zone.textContent.trim();
                        e.dataTransfer.setData('text/plain', optionId);
