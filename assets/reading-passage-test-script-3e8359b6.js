        ...
        const correctAnswers = {
            q14: "FLYNN EFFECT|THE FLYNN EFFECT",
            q15: "VISUO-SPATIAL",
            q16: "VERBAL",
            q17: "CLEVERER",
            q18: "PLATEAUED",
        ...
        const TEST_STORAGE_ID = 'reading_reading_passage_2s_passage_2_4';
        const TEST_LABEL = 'Passage 2.4';
...
function normalizeSummaryGroups() {
    document.querySelectorAll('.question-group').forEach(group => {
        ...
        el('submit-all-btn').addEventListener('click', () => {
            ...
            const results = {};
            const groupedBiophilicChoices = Array.from(document.querySelectorAll('input[name="q25_26_choice"]'));
            const groupedBiophilicSelected = groupedBiophilicChoices.filter(cb => cb.checked).map(cb => cb.value);
            ...
            for (let q = firstQuestion; q <= lastQuestion; q++) {
                ...
                if (q >= 25 && q <= 26 && groupedBiophilicChoices.length) {
                    user = groupedBiophilicSelected.join(',') || null;
                    isCorrect = groupedBiophilicSelected.includes(String(corr).toUpperCase());
                    results[key] = { userAnswer: user || 'No answer', correct: corr, isCorrect };
                    if (q === 26) groupedBiophilicChoices.forEach(cb => cb.disabled = true);
                    if (isCorrect) score++;
                    continue;
                }
        ...
        function showCorrectIndicators(results) {
            ...
            const groupedBiophilicReview = el('q25-26-review');
            if (groupedBiophilicReview) groupedBiophilicReview.innerHTML = '';
            ...
            for (let q = firstQuestion; q <= total; q++) {
                ...
                if ((q === 25 || q === 26) && groupedBiophilicReview) {
                    ...
                    groupedBiophilicReview.appendChild(box);
                    continue;
                }
