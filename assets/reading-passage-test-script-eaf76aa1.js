        ...
        const TEST_STORAGE_ID = 'reading_reading_passage_1s_passage_1_19';
        const TEST_LABEL = 'Passage 1.19';
...
const REDO_QUERY = SEARCH_PARAMS.get('redo') === '1';
        ...
        function setupTripleMultiSelect(groupName, firstTarget, secondTarget, thirdTarget) {
            const checks = Array.from(document.querySelectorAll(`input[name="${groupName}"]`));
            const first = document.querySelector(`input[name="${firstTarget}"]`);
            const second = document.querySelector(`input[name="${secondTarget}"]`);
            const third = document.querySelector(`input[name="${thirdTarget}"]`);
            if (!checks.length || !first || !second || !third) return;

            const sync = (changed) => {
                const selected = checks.filter(cb => cb.checked);
                if (selected.length > 3 && changed && changed.checked) {
                    changed.checked = false;
                }
                const final = checks.filter(cb => cb.checked).map(cb => cb.value);
                first.value = final[0] || '';
        ...
        setupTripleMultiSelect('q5_7_choice', 'q5', 'q6', 'q7');
        ...
        el('submit-all-btn').addEventListener('click', () => {
            ...
            const results = {};
            const groupedCastleChoices = Array.from(document.querySelectorAll('input[name="q5_7_choice"]'));
            const groupedCastleSelected = groupedCastleChoices.filter(cb => cb.checked).map(cb => cb.value);
            const groupedCastleCorrect = ['C', 'E', 'G'];
            ...
            for (let q = 1; q <= total; q++) {
                ...
                if (q >= 5 && q <= 7 && groupedCastleChoices.length) {
                    user = groupedCastleSelected.join(',') || null;
                    isCorrect = groupedCastleSelected.includes(String(corr).toUpperCase());
                    results[key] = { userAnswer: user || 'No answer', correct: corr, isCorrect };
                    if (q === 7) groupedCastleChoices.forEach(cb => cb.disabled = true);
                    if (isCorrect) score++;
                    continue;
                }
