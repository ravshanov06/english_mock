        ...
        const TEST_STORAGE_ID = 'reading_reading_passage_1s_passage_1_33';
        const TEST_LABEL = 'Passage 1.33';
...
const REDO_QUERY = SEARCH_PARAMS.get('redo') === '1';
        ...
        function getGroupedCheckboxContext(q) {
            if (q >= 11 && q <= 13) {
                return {
                    start: 11,
                    end: 13,
                    name: 'q11_12_13',
                    checkboxes: Array.from(document.querySelectorAll('input[name="q11_12_13"][type="checkbox"]'))
                };
            }
            return null;
        }
        function getGroupedCheckboxValues(groupedContext) {
            if (!groupedContext) return [];
            return groupedContext.checkboxes.filter((cb) => cb.checked).map((cb) => cb.value).sort();
        }
        function collectCurrentAnswers() {
            ...
            for (let q = 1; q <= maxQ; q++) {
                ...
                } else if (getGroupedCheckboxContext(q)) {
                    const values = getGroupedCheckboxValues(getGroupedCheckboxContext(q));
                    if (values.length) saved[key] = values.join(',');
        ...
        function restoreSavedAnswers() {
            ...
            Object.keys(stored).forEach((key) => {
                ...
                const groupedContext = getGroupedCheckboxContext(parseInt(key.replace('q', ''), 10));
                if (groupedContext) {
                    const selected = String(value).split(',').map((part) => part.trim()).filter(Boolean);
                    groupedContext.checkboxes.forEach((cb) => {
        ...
        function attachAnsweredListeners() {
            document.querySelectorAll('input[type="radio"], input[type="text"], select, input[type="checkbox"]').forEach(input => {
                input.addEventListener('change', () => {
                    ...
                    if (!m) {
                        if (input.name === 'q11_12_13') {
                            const values = getGroupedCheckboxValues(getGroupedCheckboxContext(11));
                            for (let q = 11; q <= 13; q++) {
                                const chip = document.querySelector(`.progress-item[data-question="${q}"]`);
                                if (chip) chip.classList.toggle('answered', values.length >= (q - 10));
                            }
                        }
                        return;
                    }
        ...
        function setupLimitedCheckboxes() {
            document.querySelectorAll('.multi-select-question').forEach((question) => {
                const limit = parseInt(question.dataset.limit || '3', 10);
                const checkboxes = Array.from(question.querySelectorAll('.multi-checkbox'));
        ...
        el('submit-all-btn').addEventListener('click', () => {
            ...
            for (let q = 1; q <= total; q++) {
                const key = `q${q}`;
                let corr = correctAnswers[key];
                const groupedContext = getGroupedCheckboxContext(q);
                ...
                if (radios.length > 0) {
                    ...
                    const values = getGroupedCheckboxValues(groupedContext);
                    user = values.join(',');
                    isCorrect = values.includes(corr);
                    groupedContext.checkboxes.forEach(cb => cb.disabled = true);
                } else if (text) {
        ...
        function showCorrectIndicators(results) {
            ...
            document.querySelectorAll('.reading-review-correct-choice,.reading-review-wrong-choice').forEach((node) => {
                node.classList.remove('reading-review-correct-choice', 'reading-review-wrong-choice');
            });
            ...
            for (let q = 1; q <= total; q++) {
                ...
                if (groupedContext) {
                    if (q !== groupedContext.start) continue;
                    const correctValues = [];
                    for (let n = groupedContext.start; n <= groupedContext.end; n++) {
                        correctValues.push(correctAnswers[`q${n}`]);
                    }
                    const userValues = getGroupedCheckboxValues(groupedContext);
                    const correctSet = new Set(correctValues);
                    const userSet = new Set(userValues);
                    groupedContext.checkboxes.forEach((cb) => {
                        const label = cb.closest('label') || cb.parentNode;
                        if (!label) return;
                        if (correctSet.has(cb.value)) label.classList.add('reading-review-correct-choice');
                        if (userSet.has(cb.value) && !correctSet.has(cb.value)) label.classList.add('reading-review-wrong-choice');
                    });

                    const target = groupedContext.checkboxes[0] && groupedContext.checkboxes[0].closest('.question');
                    if (!target) continue;
                    const box = document.createElement('div');
                    box.className = 'answer-indicator ' + (correctValues.every((value) => userSet.has(value)) && userValues.length === correctValues.length ? 'correct' : 'incorrect');
                    box.style.display = 'block';
                    box.style.marginTop = '6px';
                    box.innerHTML = box.classList.contains('correct')
                        ? `<i class="fas fa-check"></i> Correct! <strong>Answers:</strong> ${correctValues.join(', ')}`
                        : `<i class="fas fa-times"></i> Incorrect. Your answer: <strong>${userValues.join(',') || 'No answer'}</strong> <strong>Correct:</strong> ${correctValues.join(', ')}`;
        ...
        document.addEventListener('change', function (e) {
            if (e.target && e.target.name && /^q\d+(?:_\d+)*$/.test(e.target.name)) persistCurrentAnswers();
        }, true);
