    ...
    const testData = [
        {
            title: "SECTION 2",
            instructions: "Listen for the fire safety briefing and answer the multiple-choice and matching questions.",
            description: "Information for fire wardens. Questions 11-14 are multiple-choice and 15-20 match the comments A-C.",
            questionRange: "11-20"
        }
    ];
...
prepopulateForm(formPrefillAnswers);
    ...
    function showCorrectAnswers() {
        ...
        // Show grouped "choose TWO" answers (11-12, 13-14) at the group level.
        Object.entries(multiQuestionGroups).forEach(([groupKey, questionIds]) => {
            if (!Array.isArray(questionIds) || questionIds.length === 0) return;
            const container = document.getElementById(`q${questionIds[0]}`);
            if (!container) return;

            const selected = (multiAnswers[groupKey] || []).map(v => v.toString().trim().toUpperCase()).filter(Boolean);
            const correct = Array.from(new Set(
                questionIds.flatMap(questionId => getNormalizedCorrectValues(questionId))
            )).map(v => v.toString().trim().toUpperCase()).filter(Boolean);

            const correctSetLower = new Set(correct.map(v => v.toLowerCase()));
            const selectedLower = selected.map(v => v.toLowerCase());
            const correctCount = selectedLower.filter(v => correctSetLower.has(v)).length;
            const isComplete = correct.length > 0 && correctCount === correct.length && selected.length === correct.length;
            const isPartial = correctCount > 0 && !isComplete;
            ...
            feedback.style.backgroundColor = isComplete ? '#d4edda' : (isPartial ? '#fff3cd' : '#f8d7da');
            feedback.style.color = isComplete ? '#155724' : (isPartial ? '#856404' : '#721c24');

            const status = isComplete ? 'Correct.' : (isPartial ? 'Partially correct.' : 'Incorrect.');
            const userText = selected.length ? selected.join(', ') : '-';
            const correctText = correct.length ? correct.join(', ') : '-';
            var _ss = document.createElement('strong'); _ss.textContent = status; feedback.appendChild(_ss);
            feedback.appendChild(document.createTextNode(' You answered: '));
            var _eu = document.createElement('em'); _eu.textContent = userText; feedback.appendChild(_eu);
            feedback.appendChild(document.createElement('br'));
            var _sc = document.createElement('strong'); _sc.textContent = 'Correct:'; feedback.appendChild(_sc);
            feedback.appendChild(document.createTextNode(' '));
            var _ec = document.createElement('em'); _ec.textContent = correctText; feedback.appendChild(_ec);
            container.appendChild(feedback);

            questionIds.forEach(id => handled.add(id));
        });

        results.forEach(({ questionId, userAnswer, correctAnswer, isCorrect }) => {
            if (handled.has(questionId)) return;
            let container = null;
            if (questionId >= 15 && questionId <= 20) {
    ...
    function highlightAnswers() {
        ...
        const multiGroups = [
            { key: 'q11_12', questions: [11, 12] },
            { key: 'q13_14', questions: [13, 14] }
        ];
        multiGroups.forEach(group => {
            const correctSet = new Set(
                group.questions.flatMap(questionId => getNormalizedCorrectValues(questionId))
            );
            document.querySelectorAll(`input[name="${group.key}"]`).forEach(input => {
                const item = input.closest('.choice-item');
                if (!item) return;
                const inputValue = input.value.trim().toLowerCase();
                if (correctSet.has(inputValue)) {
