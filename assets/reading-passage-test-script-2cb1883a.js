        ...
        /* ========= CONFIG ========= */
        const PARTS = [
            { id: 1, title: 'Passage 2', qStart: 17, qEnd: 32, passagePanel: 'p1_passagePanel', questionsPanel: 'p1_questionsPanel' },
        ];
        ...
        const TEST_STORAGE_ID = 'reading_reading_passage_2s_passage_2_6';
        const TEST_LABEL = 'Passage 2.6';
...
function normalizeSummaryGroups() {
    document.querySelectorAll('.question-group').forEach(group => {
        ...
        const groupedChoiceConfigs = [
            { name: 'q29_30_choice', qStart: 29, qEnd: 30, reviewId: 'q29-30-review' },
            { name: 'q31_32_choice', qStart: 31, qEnd: 32, reviewId: 'q31-32-review' },
        ];
        ...
        el('submit-all-btn').addEventListener('click', () => {
            ...
            const groupedChoiceState = groupedChoiceConfigs.map(cfg => ({
                ...cfg,
                checks: Array.from(document.querySelectorAll(`input[name="${cfg.name}"]`)),
            ...
            for (let q = firstQuestion; q <= lastQuestion; q++) {
                ...
                const groupedCfg = groupedChoiceState.find(cfg => q >= cfg.qStart && q <= cfg.qEnd && cfg.checks.length);
                if (groupedCfg) {
                    user = groupedCfg.selected.join(',') || null;
                    isCorrect = groupedCfg.selected.includes(String(corr).toUpperCase());
                    results[key] = { userAnswer: user || 'No answer', correct: corr, isCorrect };
                    if (q === groupedCfg.qEnd) groupedCfg.checks.forEach(cb => cb.disabled = true);
                    if (isCorrect) score++;
                    continue;
                }
        ...
        function showCorrectIndicators(results) {
            ...
            const groupedReviews = new Map(groupedChoiceConfigs.map(cfg => [cfg.reviewId, el(cfg.reviewId)]));
            groupedReviews.forEach(review => { if (review) review.innerHTML = ''; });
            ...
            for (let q = firstQuestion; q <= total; q++) {
                ...
                const groupedCfg = groupedChoiceConfigs.find(cfg => q >= cfg.qStart && q <= cfg.qEnd);
                const groupedReview = groupedCfg ? groupedReviews.get(groupedCfg.reviewId) : null;
                if (groupedCfg && groupedReview) {
                    ...
                    box.innerHTML = res.isCorrect
                        ...
                        : `<i class="fas fa-times"></i> Incorrect. Your answer: <strong>${res.userAnswer}</strong> <strong>Correct:</strong> ${res.correct}`;
                    groupedReview.appendChild(box);
                    continue;
                }
        ...
        setupPairedMultiSelect('q29_30_choice', 'q29', 'q30');
        setupPairedMultiSelect('q31_32_choice', 'q31', 'q32');
